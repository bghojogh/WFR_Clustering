...
for i in range(n):
    for j in range(i+1, n):  # only upper triangle to avoid double drawing
        if adjacency[i, j]:
            ax.plot([X[i, 0], X[j, 0]], [X[i, 1], X[j, 1]], color='gray', alpha=0.8, linewidth=1.5)

# draw points:
X_inlier = X[labels!=-1, :]
X_outlier = X[labels==-1, :]
labels_inlier = labels[labels!=-1]
ax.scatter(X_inlier[:,0], X_inlier[:,1], c=labels_inlier, s=40, cmap=custom_cmap)
ax.scatter(X_outlier[:,0], X_outlier[:,1], c="black", s=40)

ax.set_title(f"τ={resemblance_threshold:.2f}", fontsize=25)
plt.xticks(())
plt.yticks(())
